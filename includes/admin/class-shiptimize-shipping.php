...
class ShiptimizeShipping {
    ...
    public function actions() {
        ...
        add_action( 'wp_ajax_nopriv_shiptimize_selected_carrier', array( $this, 'ajax_get_selected_carrier' ) );
        add_action( 'wp_ajax_shiptimize_shiptimize_selected_carrier', array( $this, 'ajax_get_selected_carrier' ) );

        add_action( 'wp_ajax_nopriv_shiptimize_selected_carrier_from_list', array( $this, 'ajax_get_selected_carrier_from_list' ) );
        add_action( 'wp_ajax_shiptimize_shiptimize_selected_carrier_from_list', array( $this, 'ajax_get_selected_carrier_from_list' ) );

        add_action( 'wp_ajax_nopriv_shiptimize_set_pickup_point', array( $this, 'ajax_set_pickup_point' ) );
        add_action( 'wp_ajax_shiptimize_set_pickup_point', array( $this, 'ajax_set_pickup_point' ) );
        ...
        add_action('woocommerce_store_api_checkout_order_processed',array( $this, 'shiptimize_block_order_submited'));
    ...
    public function ajax_get_selected_carrier_from_list()
    {
        $selected_carrier = filter_input( INPUT_GET, 'selected_carrier_code' );

        $carrier = $this->get_shiptimize_carrier($selected_carrier);
        ...
        if($carrier && $HasPickup) {
            ...
            $settings = $this->get_settings_from_shipping_method_id($selected_carrier);
            ...
            if($pickupbehaviour != ShiptimizeOrder::$PICKUP_BEHAVIOUR_IMPOSSIBLE) {
                echo json_encode(['status' => 1,'carrier_id' => $carrier->Id]);die();
            }
        }
    ...
    public function ajax_set_pickup_point()
    {
        ...
        WC()->session->set('shipping_pickup_id', filter_input( INPUT_GET, 'shipping_pickup_id' ));
        WC()->session->set('shipping_carrier_id',filter_input( INPUT_GET, 'shipping_carrier_id' ));
        WC()->session->set('shipping_method',WC()->session->get( 'chosen_shipping_methods' ));
        WC()->session->set('shipping_pickup_label',filter_input( INPUT_GET, 'shipping_pickup_label' ));
        WC()->session->set('shiptimize_pickup_extended',filter_input( INPUT_GET, 'shiptimize_pickup_extended' ));
    ...
    public function shiptimize_block_order_submited($order)
    {
        global $wpdb;
        $shipping_pickup_id = WC()->session->get('shipping_pickup_id' );
        $pickup_label = WC()->session->get('shipping_pickup_label' );
        $pickup_extended = WC()->session->get('shiptimize_pickup_extended' );
        ...
        if ( !is_null( $shipping_pickup_id) && !is_null( $pickup_label)  && !is_null($pickup_extended ) ) {
            // returns WC_Order object.


            $order->update_meta_data('shiptimizepickup', sanitize_text_field( $shipping_pickup_id ));
        ...
        WC()->session->__unset('shiptimizepickuplabel' );
        WC()->session->__unset('shiptimizepickupextended' );
        ...
        if( isset($choosen_methods_woo[0]) ) {
            ...
            if($carrier && $HasPickup) {
                ...
                if($pickupbehaviour != ShiptimizeOrder::$PICKUP_BEHAVIOUR_IMPOSSIBLE) {
                    ...
                    $shiptimize_options  .=  "
                <button class='button alt shiptimize-pick-location' type='button' onclick='shiptimize.getPickupLocations(event)''>" .$shiptimize->translate('Choose Pickup Location') . "</button> 
