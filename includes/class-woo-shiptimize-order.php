...
//      Check if Carrier does not match , carrier exists , ID is not 0 and ID != Transporter
     ...
     * return summary object containing the number of
     */
    public static function export ( $orders2Export, $try = 0, $printLabel = false) {
        global $wpdb;
        ##DJDJ Ovdeeee
        ...
        if (count($shiptimize_orders)) {
            if ($printLabel){
                $response = $api->post_shipments($shiptimize_orders,$order->getClientReferenceCode());
            }
            else{
        ...
        if (count($shiptimize_patch_orders)) {
            if ($printLabel){
                $response = $api->patch_shipments($shiptimize_patch_orders,$order->getClientReferenceCode());
            }else{
    ...
    public static function shipmentsResponse($response, $printLabel = false)
    {
        ...
        if (isset($response->response->Shipment)) {
            foreach ($response->response->Shipment as $shipment) {
                ...
                $orderReferenceCode = $shipment->ClientReferenceCode;
                ...
                //          Check if Carrier does not match , carrier exists , ID is not 0 and ID != Transporter
                if (isset($shipment->CarrierSelect) && $shipment->CarrierSelect->Id && $shipment->CarrierSelect->Id != $order->get_carrier_id()) {
                    $order->add_message($order->get_formated_message($shiptimize->__("Diferent carrier selected by the api")));
                }
    ...
    public static function getStatusLabel($status)
    {
        ...
        switch ($status) {
            case ShiptimizeOrder::$STATUS_NOT_EXPORTED:
                return $shiptimize->__('Not Exported');

            case ShiptimizeOrder::$STATUS_EXPORTED_SUCCESSFULLY:
                return $shiptimize->__('Exported');

            case ShiptimizeOrder::$STATUS_EXPORT_ERRORS:
                return $shiptimize->__('Error on Export');

            default:
                return $shiptimize->__("Unknown status of id"). ' ' . $status;
        }
    }
}